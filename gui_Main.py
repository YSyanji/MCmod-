...
def mod_list():   #mod列表显示
    ...
    for m in range(len(list_ls)):
        ...
        exec(f'Label_text_{m}_1 = tk.Label(Label_text_{m},width=22,image=photo)')
...
Modulemain.attributes("-alpha",0.9) #窗口透明度 -alpha
...
close_window.grid(column = 3,row = 3,padx =5,pady =5)
#--------------------------关闭窗口
...
comb_1.grid(column = 0,row = 1,padx =5)
comb_1.set("1.21.x")
#--------------------------版本
...
text_2.grid(column = 1,row = 0,padx =5)
...
comb_2.grid(column = 1,row = 1,padx =5)
comb_2.set("Forge")
#--------------------------运行方式
...
text_3.grid(column = 2,row =0 ,padx =5)
